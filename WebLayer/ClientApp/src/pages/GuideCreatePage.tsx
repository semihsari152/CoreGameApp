...
const GuideCreatePage: React.FC = () => {
  ...
  const handleTagAdd = () => {
    ...
    if (trimmedTag.length > 20) {
      toast.error('Tag en fazla 20 karakter olabilir');
      return;
    }
  ...
  const onSubmit = async (data: GuideFormData) => {
    if (!isAuthenticated) {
      toast.error('Rehber oluşturmak için giriş yapmalısınız');
      return;
    }
    ...
    try {
      const response = await fetch('/api/guides', {
        ...
        body: JSON.stringify({
          ...
          userId: user?.id || 0,
          isPublished: true, // Always publish directly
          tags: selectedTags, // Send selected tags as string array
          guideBlocks: blocks.filter(block => block.content?.trim() || block.mediaUrl?.trim() || block.blockType === GuideBlockType.Divider),
        })
      });
      ...
      if (response.ok) {
        ...
        const guideId = result?.data?.id || result?.id || Math.floor(Math.random() * 1000);
        ...
        let error;
        try {
          error = responseText ? JSON.parse(responseText) : { message: 'Bilinmeyen hata' };
        } catch {
          error = { message: 'Sunucu hatası' };
        }
        toast.error(error.message || 'Rehber oluşturulurken bir hata oluştu');
      }
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-dark-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Rehber oluşturmak için giriş yapmalısınız.
          </p>
        </div>
      </div>
    );
        ...
        {previewMode ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="max-w-4xl mx-auto space-y-8">
                ...
                <div>
                  ...
                  <input
                    {...register('title', {
                      ...
                      minLength: {
                        value: 15,
                        message: 'Başlık en az 15 karakter olmalıdır'
                      },
                      maxLength: {
                        value: 100,
                ...
                <div>
                  ...
                  <textarea
                    {...register('summary', {
                      ...
                      minLength: {
                        value: 30,
                        message: 'Özet en az 30 karakter olmalıdır'
                      },
                      maxLength: {
                        value: 350,
                        message: 'Özet en fazla 350 karakter olabilir'
                      }
                    })}
                    rows={3}
                    className={`input resize-none ${errors.summary ? 'border-red-500' : ''}`}
                    placeholder="Rehberinizin kısa bir özetini yazın... (30-350 karakter)"
                  />
                  {errors.summary && (
                    <p className="mt-1 text-sm text-red-600 dark:text-red-400">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <Save className="w-4 h-4 mr-2" />
                {isSubmitting ? 'Oluşturuluyor...' : 'Rehberi Oluştur'}
              </button>
            </div>
          </form>
        )}
