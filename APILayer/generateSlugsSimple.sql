-- Simple slug generation for existing data

-- Update BlogPosts with basic slugs
UPDATE BlogPosts 
SET Slug = LOWER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(Title, ' ', '-'), 'ç', 'c'), 'ğ', 'g'), 'ı', 'i'), 'ö', 'o'), 'ş', 's'), 'ü', 'u'), '?', '')) + '-' + CAST(Id as VARCHAR(10))
